...
const ofertasIniciais: Oferta[] = [
  { codigo: "AED401", nome: "Compiladores", turma: "T01", prof: "Carlos Lima", horario: "Seg/Qua 08-10", creditos: 4, vagas: "12/30", status: "Disponível" },
  { codigo: "ES401", nome: "Arquitetura de Software", turma: "T01", prof: "Ana Souza", horario: "Ter/Qui 10-12", creditos: 4, vagas: "20/30", status: "Disponível" },
  { codigo: "IA401", nome: "Inteligência Artificial", turma: "T01", prof: "Lia Mendes", horario: "Ter/Qui 14-16", creditos: 4, vagas: "8/25", status: "Disponível" },
  { codigo: "RC405", nome: "Computação em Nuvem", turma: "T01", prof: "Pedro Alves", horario: "Sex 14-18", creditos: 4, vagas: "25/25", status: "Sem vaga" },
  { codigo: "BD402", nome: "Big Data", turma: "T01", prof: "Marcos R.", horario: "Qua 14-18", creditos: 4, vagas: "10/30", status: "Disponível" },
  { codigo: "ES501", nome: "Eng. de Software III", turma: "T01", prof: "Júlia P.", horario: "Seg/Qua 10-12", creditos: 4, vagas: "15/30", status: "Disponível" },
];
...
function Page() {
  ...
  return (
    <AppShell title="Matrícula" subtitle={subtitle}>
      ...
      {perfil === "estudante" && view.kind === "trancarDisc" && <TrancarDisciplina onBack={() => setView({ kind: "overview" })} />}
      {perfil === "estudante" && view.kind === "trancarPeriodo" && <TrancarPeriodo onBack={() => setView({ kind: "overview" })} />}
      {perfil === "estudante" && view.kind === "excecao" && (
        <Excecao
          disciplina={view.disciplina}
          onBack={() => setView({ kind: "wizard", step: 1 })}
        />
      )}
    </AppShell>
  );
...
function Wizard({
  step, ofertas, selecionadas, creditos, temConflito, onToggle, onStep, onCancel, onExcecao,
}: {
  step: 0 | 1 | 2 | 3;
  ofertas: Oferta[];
  selecionadas: Oferta[];
    ...
    <div className="space-y-5">
        ...
        <>
          <div className="flex flex-wrap items-center gap-6 rounded-xl border bg-card p-4 text-[13px] shadow-card">
            ...
            <div className="ml-auto flex gap-2">
              <Button disabled={selecionadas.length === 0} onClick={() => onStep(1)}>Validar plano</Button>
            </div>
          </div>
          <DataTable
            columns={[
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          {temConflito && (
            <ValidationCallout tone="error">Choque de horário entre ES401 e IA401 às terças 10–12.</ValidationCallout>
          )}
          {creditos > 24 && (
            <ValidationCallout tone="error">Você excedeu o limite de 24 créditos.</ValidationCallout>
          )}
      ...
      {step === 3 && (
        <div className="space-y-4">
          <SuccessBanner title="Matrícula 2026.1 confirmada!" description={`${selecionadas.length} disciplinas · ${creditos} créditos. Protocolo MAT-2026-0012.`} />
...
function Ajuste({ onBack }: { onBack: () => void }) {
  return (
    <div className="space-y-4">
      ...
      <SectionTitle title="Ajuste de matrícula" subtitle="Janela aberta até 23/03/2025. Inclusões e exclusões são imediatas." />
      <DataTable
        columns={[
          { key: "codigo", header: "Código" }, { key: "nome", header: "Disciplina" },
          { key: "acao", header: "Ação", render: (r) => <StatusBadge tone={r.acao === "Incluir" ? "success" : "danger"}>{r.acao}</StatusBadge> },
          { key: "acoes", header: "", align: "right", render: (r) => <RowActionButton tone="danger" onClick={() => toast.success(`Ajuste em ${r.codigo} revertido.`)}>Reverter</RowActionButton> },
        ]}
        rows={[
      ...
      />
      <div className="flex justify-end gap-2"><Button variant="outline" onClick={onBack}>Cancelar</Button><Button>Confirmar ajustes</Button></div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card p-6 shadow-card">
        ...
        <div className="mt-4 flex justify-end gap-2"><Button variant="outline" onClick={onBack}>Cancelar</Button><Button variant="destructive">Trancar disciplina</Button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card p-6 shadow-card">
        ...
        <div className="mt-4 flex justify-end gap-2"><Button variant="outline" onClick={onBack}>Voltar</Button><Button variant="destructive">Trancar período</Button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card p-6 shadow-card">
        ...
        <div className="mt-4 flex justify-end gap-2"><Button variant="outline" onClick={onBack}>Cancelar</Button><Button>Enviar solicitação</Button></div>
      </div>
    </div>
  );
}
