...
while true; do
  ...
  if find . -name '*.java' -path '*/src/main/java/*' -newer /tmp/src_stamp 2>/dev/null | grep -q .; then
    ...
    mvn compile -pl dominio-compartilhado,dominio-matricula,dominio-mobilidade-academica,aplicacao,infraestrutura,apresentacao-backend -DskipTests -q 2>&1 | tail -5
    touch /tmp/src_stamp
