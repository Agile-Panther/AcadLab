...
const matrizesIniciais: Matriz[] = [
  {
    id: "M2020",
    nome: "Eng. de Software — Matriz 2020",
    versao: "2020.1",
    status: "Ativa",
    disciplinas: 48,
    cargaH: 3600,
  ...
  {
    id: "M2024",
    nome: "Eng. de Software — Matriz 2024",
    versao: "2024.1",
    status: "Rascunho",
    disciplinas: 52,
    cargaH: 3720,
  ...
  {
    id: "M2015",
    nome: "Eng. de Software — Matriz 2015",
    versao: "2015.2",
    status: "Inativa",
    disciplinas: 46,
    cargaH: 3500,
  ...
  const { active: perfil } = useProfileSwitcher([
    {
      ...
      description: "Edita matrizes e disciplinas",
    },
    {
      value: "dra",
      label: "DRA / Registro Acadêmico",
      description: "Audita versões e equivalências",
    },
  ]);
    ...
    <AppShell title="Gestão Curricular" subtitle={subtitle}>
      {!atual ? (
        <div className="space-y-5">
          <StatsRow
            stats={[
              ...
              {
                label: "Ativa",
                value: matrizes.filter((m) => m.status === "Ativa").length,
                tone: "success",
              },
              {
                label: "Em rascunho",
                value: matrizes.filter((m) => m.status === "Rascunho").length,
                tone: "warning",
              },
          ...
          {perfil === "dra" && (
            <ValidationCallout tone="info">
              Visão DRA: somente leitura. Use a coluna "Abrir" para auditar equivalências e cargas
              horárias.
            </ValidationCallout>
          )}
          <DataTable
        ...
        <DetalheMatriz
          ...
          onToggle={() => toggleStatus(atual.id)}
        />
      )}
    </AppShell>
  );
  ...
  return (
    <div className="space-y-5">
      <Button variant="ghost" size="sm" onClick={onBack}>
        <ArrowLeft className="mr-1 h-4 w-4" /> Matrizes
      </Button>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <SectionTitle
          title={matriz.nome}
          subtitle={`Versão ${matriz.versao} · ${matriz.disciplinas} disciplinas`}
        />
      ...
      {readOnly && (
        <ValidationCallout tone="info">
          Visão DRA · Auditoria curricular. Apenas a Coordenação pode alterar matrizes.
        </ValidationCallout>
      )}

      {!editavel && (
        <ValidationCallout tone="info">
          Matriz ativa: a estrutura de disciplinas não pode ser alterada. Desative para editar.
        </ValidationCallout>
      )}
          ...
          <SectionTitle
            ...
            right={
              editavel ? (
                <Button
                  size="sm"
                  onClick={() => toast.success("Disciplina adicionada à matriz (rascunho).")}
                >
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: "nome", header: "Disciplina" },
              {
                ...
                header: "Tipo",
                render: (r) => (
                  <StatusBadge tone={r.tipo === "Obrigatória" ? "info" : "neutral"}>
                    {r.tipo}
                  </StatusBadge>
                ),
              },
              ...
              { key: "preReq", header: "Pré-req" },
              {
                ...
                render: (r) =>
                  editavel ? (
                    <RowActionButton
                      tone="danger"
                      onClick={() => toast.success(`${r.cod} removida da matriz (rascunho).`)}
                    >
                      Remover
                    </RowActionButton>
                  ) : (
                    <RowActionButton
                      tone="neutral"
                      onClick={() => toast.info(`Detalhes de ${r.cod} — ${r.nome}`)}
                    >
                      Ver
                    </RowActionButton>
                  ),
        ...
        <div className="space-y-4">
          <div className="rounded-xl border bg-card p-5 shadow-card">
            ...
            <div className="mt-3 space-y-3">
              ...
              <ProgressRow
                label="Créditos"
                current={matriz.creditos}
                total={240}
                unit="cr"
                tone="success"
              />
              <ProgressRow
                label="Obrigatórias cadastradas"
                current={42}
                total={42}
                unit=""
                tone="success"
            ...
            <div className="rounded-xl border bg-card p-5 shadow-card">
              ...
              <div className="mt-3 grid grid-cols-1 gap-3">
                <FormField label="Disciplina">
                  <Input className="h-10" placeholder="ES401" />
                </FormField>
                <FormField label="Pré-requisito">
                  <Input className="h-10" placeholder="AED301" />
                </FormField>
                <Button
                  className="w-full"
                  onClick={() =>
              ...
              <ValidationCallout tone="info" className="mt-3">
                O sistema valida automaticamente ciclos no grafo de pré-requisitos.
              </ValidationCallout>
            </div>
          )}
        </div>
