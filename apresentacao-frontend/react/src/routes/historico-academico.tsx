...
const solicitacoesIniciais: Solicitacao[] = [
  {
    id: "HIS-2025-041",
    aluno: "Maria Santos",
    matricula: "2021.10245",
    disciplina: "AED301 — Algoritmos Avançados",
    periodo: "2024.2",
    motivo: "Nota lançada divergente da prova final.",
    aberta: "12/03/2025",
    status: "Em análise",
  },
  {
    id: "HIS-2025-042",
    aluno: "Pedro Almeida",
    matricula: "2022.20988",
    disciplina: "BD302 — Banco de Dados II",
    periodo: "2024.2",
    motivo: "Aproveitamento externo não registrado.",
    aberta: "14/03/2025",
    status: "Em análise",
  },
  {
    id: "HIS-2025-038",
    aluno: "Júlia Rocha",
    matricula: "2020.10112",
    disciplina: "IA101 — Intro. à IA",
    periodo: "2023.2",
    motivo: "Frequência incorreta lançada.",
    aberta: "10/03/2025",
  ...
  },
  {
    id: "HIS-2025-031",
    aluno: "Lucas Pires",
    matricula: "2021.10455",
    disciplina: "ENG201 — Engenharia de Software",
    periodo: "2024.1",
    motivo: "Equivalência não aplicada.",
    aberta: "02/03/2025",
    status: "Deferida",
  },
];
...
function Page() {
  const { active: perfil } = useProfileSwitcher([
    { value: "estudante", label: "Estudante", description: "Consulta e solicita correções" },
    {
      ...
      description: "Audita históricos e julga correções",
    },
  ]);
  ...
  if (
    !isSec &&
    (view.kind === "sec-list" || view.kind === "sec-detail" || view.kind === "sec-aluno")
  ) {
    ...
    <AppShell title="Histórico Acadêmico" subtitle={subtitle}>
      ...
      {!isSec && view.kind === "overview" && (
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={() => setView({ kind: "emitir" })}>
              <Download className="mr-2 h-4 w-4" /> Emitir histórico oficial
            </Button>
            <Button variant="outline" onClick={() => setView({ kind: "correcao" })}>
              <FileSearch className="mr-2 h-4 w-4" /> Solicitar correção
            </Button>
          </div>
          ...
          <SectionTitle
            ...
            subtitle="Clique em um período para ver as disciplinas cursadas."
          />
          <div className="space-y-3">
            {periodos.map((p) => (
              <div key={p.p} className="rounded-xl border bg-card shadow-card">
                <button
                  onClick={() => setExpanded(expanded === p.p ? null : p.p)}
                  className="flex w-full items-center justify-between p-4 text-left"
                >
                  <div>
                    <p className="font-semibold text-foreground">{p.p}</p>
                    <p className="text-[12px] text-muted-foreground">
                      {p.aprov} aprovações · {p.reprov} reprovações · CR {p.cr}
                    </p>
                  </div>
                  <span className="text-[12px] text-muted-foreground">
                    {expanded === p.p ? "Recolher" : "Expandir"}
                  </span>
                </button>
                {expanded === p.p && (
      ...
      {!isSec && view.kind === "correcao" && (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border bg-card p-6 shadow-card">
            <SectionTitle
              title="Solicitar correção de histórico"
              subtitle="Sua solicitação será analisada pela Secretaria Acadêmica."
            />
            <div className="mt-4 grid grid-cols-2 gap-4">
              <FormField label="Disciplina" required>
                <Input className="h-10" placeholder="AED301 — Algoritmos Avançados" />
              </FormField>
              <FormField label="Período" required>
                <Input className="h-10" placeholder="2024.2" />
              </FormField>
              <FormField label="O que precisa ser corrigido" full required>
                <Textarea rows={4} placeholder="Descreva a inconsistência" />
              </FormField>
            ...
            <ValidationCallout className="mt-4" tone="info">
              A correção será efetivada apenas após deferimento com registro do responsável.
            </ValidationCallout>
            <div className="mt-4 flex justify-end gap-2">
              ...
              <Button
                onClick={() => {
                  toast.success("Solicitação enviada à Secretaria Acadêmica.");
                  setView({ kind: "overview" });
                }}
              >
                Enviar solicitação
      ...
      {!isSec && view.kind === "emitir" && (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border bg-card p-6 shadow-card">
            <SectionTitle
              title="Emitir histórico oficial"
              subtitle="Documento inclui apenas períodos encerrados e consolidados."
            />
            <ul className="mt-4 space-y-2 text-[13px] text-foreground">
              ...
              <li className="text-muted-foreground">
                Período 2025.2 (em andamento) não será incluído.
              </li>
            </ul>
            <div className="mt-4 flex justify-end gap-2">
              <Button variant="outline" onClick={() => setView({ kind: "overview" })}>
              ...
              <Button
                onClick={() => {
                  toast.success("Histórico oficial gerado e baixado.");
                  setView({ kind: "overview" });
                }}
              >
      ...
      {isSec && view.kind === "sec-list" && (
        <div className="space-y-5">
            ...
            stats={[
              {
                ...
                value: solicitacoes.filter(
                  (s) => s.status === "Em análise" || s.status === "Pendente complemento",
                ).length,
                tone: "warning",
              },
              {
                label: "Em análise",
                value: solicitacoes.filter((s) => s.status === "Em análise").length,
                tone: "info",
              },
              {
                label: "Pendentes complemento",
                value: solicitacoes.filter((s) => s.status === "Pendente complemento").length,
                tone: "danger",
              },
              {
                label: "Deferidas (mês)",
                value: solicitacoes.filter((s) => s.status === "Deferida").length,
                tone: "success",
              },
            ]}
          />
          ...
          <SectionTitle
            title="Fila de correções de histórico"
            subtitle="Solicitações abertas pelos estudantes aguardando triagem."
          />
          <DataTable
            columns={[
              ...
              },
              {
                ...
                align: "right",
                render: (r) => (
                  <RowActionButton onClick={() => setView({ kind: "sec-detail", id: r.id })}>
                    Analisar
                  </RowActionButton>
                ),
              },
          ...
          <SectionTitle
            title="Consulta de históricos por estudante"
            subtitle="Localize e emita o histórico oficial de qualquer estudante."
          />
          ...
          <DataTable
            columns={[
              ...
              { key: "cr", header: "CR", align: "right" },
              {
                ...
                header: "Situação",
                render: (r) => (
                  <StatusBadge tone={r.situacao === "Ativo" ? "success" : "warning"}>
                    {r.situacao}
                  </StatusBadge>
                ),
              },
              {
                ...
                render: (r) => (
                  <RowActionButton
                    onClick={() => setView({ kind: "sec-aluno", matricula: r.matricula })}
                  >
                    Abrir histórico
                  </RowActionButton>
                ),
      ...
      {isSec &&
        view.kind === "sec-detail" &&
        (() => {
          const s = solicitacoes.find((x) => x.id === view.id);
          if (!s) return null;
          return (
            <div className="space-y-4">
              <Button variant="ghost" size="sm" onClick={() => setView({ kind: "sec-list" })}>
                <ArrowLeft className="mr-1 h-4 w-4" /> Voltar à fila
              </Button>
              <div className="rounded-xl border bg-card p-6 shadow-card">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <SectionTitle
                      ...
                      subtitle={`${s.aluno} · Matrícula ${s.matricula}`}
                    />
                  </div>
                  <StatusBadge
                    tone={
                ...
                <div className="mt-4 grid grid-cols-2 gap-4 text-[13px]">
                  <div>
                    ...
                    <span className="text-foreground">{s.disciplina}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Período:</span>{" "}
                    <span className="text-foreground">{s.periodo}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Aberta em:</span>{" "}
                    <span className="text-foreground">{s.aberta}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Anexos:</span>{" "}
                    <span className="text-foreground">1 arquivo (boletim_assinado.pdf)</span>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  ...
                  <p className="mt-1 rounded-lg border border-border bg-muted/30 p-3 text-[13px] text-foreground">
                    {s.motivo}
                  </p>
                </div>
                ...
                <div className="mt-5">
                  ...
                  <Textarea
                    className="mt-1"
                    rows={3}
                    placeholder="Descreva o parecer registrado no protocolo..."
                  />
                </div>

                <ValidationCallout className="mt-4" tone="info">
                  Toda alteração no histórico exige registro do responsável e fica auditável no log
                  do estudante.
                </ValidationCallout>
                ...
                <div className="mt-4 flex flex-wrap justify-end gap-2">
                  <Button
                    variant="outline"
                    onClick={() =>
                      decidir(s.id, "Pendente complemento", "marcada como pendente de complemento")
                    }
                  >
                  ...
                  <Button
                    variant="outline"
                    onClick={() => decidir(s.id, "Indeferida", "indeferida")}
                  >
                    <X className="mr-2 h-4 w-4" /> Indeferir
                  </Button>
                  <Button
                    onClick={() => decidir(s.id, "Deferida", "deferida e histórico atualizado")}
                  >
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border bg-card p-6 shadow-card">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <StatusBadge tone={a.situacao === "Ativo" ? "success" : "warning"}>
                    {a.situacao}
                  </StatusBadge>
                </div>
                <StatsRow
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  ...
                  <Button
                    variant="outline"
                    onClick={() => toast.success("Declaração de matrícula gerada.")}
                  >
                    Emitir declaração
                  </Button>
                </div>
                ...
                <DataTable
                  columns={[
                    ...
                    { key: "cr", header: "CR período", align: "right" },
                    {
                      ...
                      align: "right",
                      render: () => (
                        <RowActionButton onClick={() => toast("Detalhe do período (mock).")}>
                          Ver disciplinas
                        </RowActionButton>
                      ),
                    },
