      ...
      <div className="space-y-5">
        ...
        <div className="rounded-xl border bg-card p-5 shadow-card">
          <SectionTitle
            ...
            subtitle="Atualizado a partir do histórico consolidado."
          />
          <div className="mt-3 space-y-3">
            <ProgressRow label="Disciplinas obrigatórias" current={32} total={42} tone="info" />
            <ProgressRow
              label="Carga horária optativa"
              current={360}
              total={600}
              unit="h"
              tone="warning"
        ...
        {status === "Não solicitada" && (
          <div className="rounded-xl border bg-card p-6 text-center shadow-card">
            ...
            <Button className="mt-4" onClick={() => setStatus("Em análise")}>
              <FileSearch className="mr-2 h-4 w-4" /> Solicitar análise de conclusão
            </Button>
            <ValidationCallout className="mt-4" tone="info">
              A solicitação só pode ser iniciada após o encerramento do último período cursado.
            </ValidationCallout>
          </div>
        )}
            ...
            <div className="rounded-xl border bg-card p-5 shadow-card">
              <div className="flex items-center justify-between">
                <SectionTitle
                  ...
                  subtitle="A Secretaria está gerando seu checklist. Prazo: 10 dias úteis."
                />
                <StatusBadge tone="warning">Em análise</StatusBadge>
              </div>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setStatus("Apto")}>
                (simular) Apto
              </Button>
              <Button variant="outline" onClick={() => setStatus("Inapto")}>
                (simular) Inapto
              </Button>
            </div>
          </div>
        ...
        {status === "Apto" && (
          <div className="space-y-4">
            <SuccessBanner
              ...
              description="Aprovado pelo Coordenador Acadêmico em 22/03/2025. Aguarde a data da cerimônia."
            />
            <div className="rounded-xl border bg-card p-5 shadow-card">
        ...
        {status === "Inapto" && (
          <div className="space-y-4">
            ...
            <DataTable
              columns={[
                { key: "req", header: "Requisito" },
                { key: "pend", header: "Pendência" },
                {
                  key: "acao",
                  header: "",
                  align: "right",
                  render: (r) => (
                    <RowActionButton onClick={() => toast.info(`Orientação: ${r.pend}`)}>
                      Como resolver
                    </RowActionButton>
                  ),
                },
            ...
            />
            <Button variant="ghost" onClick={() => setStatus("Não solicitada")}>
    ...
    <div className="space-y-5">
      <StatsRow
        stats={[
          ...
          {
            label: "Aptos",
            value: formandos.filter((f) => f.status === "Apto").length,
            tone: "success",
          },
          {
            label: "Pendentes",
          ...
          {
            label: "Inaptos",
            value: formandos.filter((f) => f.status === "Inapto").length,
            tone: "danger",
          },
        ]}
      />
      <SectionTitle
        title="Análise da turma de formandos 2025.2"
        subtitle="Audite a integralização e libere a colação de grau."
      />
      <DataTable
        columns={[
          ...
          { key: "pend", header: "Pendências", align: "right" },
          {
            ...
            render: (r) => (
              <StatusBadge
                tone={
                  r.status === "Apto" ? "success" : r.status === "Pendente" ? "warning" : "danger"
                }
              >
                {r.status}
              </StatusBadge>
          ...
          },
          {
            ...
            render: (r) =>
              r.status === "Apto" ? (
                ...
                <RowActionButton tone="info" onClick={() => liberar(r.matricula)}>
                  Liberar colação
                </RowActionButton>
              ),
          },
      ...
      <DataTable
        columns={[
          ...
          { key: "data", header: "Colação" },
          {
            ...
            header: "Status",
            render: (r) => (
              <StatusBadge tone={r.status === "Emitido" ? "success" : "warning"}>
                {r.status}
              </StatusBadge>
            ),
          },
          {
            ...
            render: (r) =>
              r.status === "Emitido" ? (
                <RowActionButton onClick={() => toast.success("Diploma reemitido.")}>
                  Reemitir
                </RowActionButton>
              ) : (
                <RowActionButton
                  tone="info"
                  onClick={() => toast.success("Diploma emitido com sucesso.")}
                >
                  Emitir diploma
                </RowActionButton>
              ),
