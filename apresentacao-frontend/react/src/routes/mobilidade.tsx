...
function Page() {
  ...
  const { active: perfil } = useProfileSwitcher([
    ...
    {
      value: "coordenador",
      label: "Coord. Mobilidade",
      description: "Aprova e homologa equivalências",
    },
  ]);
  const subtitle =
    perfil === "coordenador"
    ...
    <AppShell title="Mobilidade Acadêmica" subtitle={subtitle}>
        ...
        <div className="space-y-5">
          <StatsRow
            stats={[
              ...
              { label: "Disciplinas no plano", value: 4, tone: "info" },
              {
                ...
                value: mob.status,
                tone:
                  mob.status === "Autorizada" || mob.status === "Concluída" ? "success" : "warning",
              },
            ]}
          />
          ...
          <div className="rounded-xl border bg-card p-5 shadow-card">
            ...
            <DataTable
              ...
              columns={[
                ...
                { key: "ch", header: "CH ext.", align: "right" },
                { key: "equiv", header: "Equivalência local" },
                { key: "chL", header: "CH local", align: "right" },
                {
                  key: "status",
                  header: "Status",
                  render: (r) => (
                    <StatusBadge tone={r.status === "Aprovada" ? "success" : "warning"}>
                      {r.status}
                    </StatusBadge>
                  ),
                },
              ...
              rows={[
                {
                  ext: "Compiladores",
                  ch: 80,
                  equiv: "AED401 — Compiladores",
                  chL: 80,
                  status: "Aprovada",
                },
                {
                  ext: "Engenharia de Software",
                  ch: 60,
                  equiv: "ES401 — Arq. de Software",
                  chL: 60,
                  status: "Aprovada",
                },
                {
                  ext: "Ética em Computação",
                  ch: 30,
                  equiv: "Optativa Livre",
                  chL: 30,
                  status: "Aprovada",
                },
                {
                  ext: "Linguística Aplicada",
                  ch: 60,
                  equiv: "—",
                  chL: 0,
                  status: "Não equivalente",
                },
              ]}
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {mob.status === "Autorizada" && (
              <>
                <Button variant="outline" onClick={() => setView("comprovantes")}>
                  Anexar comprovantes
                </Button>
                <Button variant="destructive" onClick={handleCancelMob}>
                  Cancelar mobilidade
                </Button>
              </>
            )}
            {mob.status === "Em curso" && (
              <Button onClick={() => setView("comprovantes")}>Anexar comprovantes</Button>
            )}
          </div>
        </div>
      )}
      ...
      {view === "wizard" && (
        <Wizard
          ...
          onDone={() => {
            setView("overview");
            setMob({ instituicao: "Nova", pais: "—", inicio: "—", fim: "—", status: "Em análise" });
          }}
        />
      )}
      ...
      {view === "comprovantes" && (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border bg-card p-6 shadow-card">
            <SectionTitle
              title="Anexar comprovantes de conclusão"
              subtitle="Necessários para que a secretaria registre os resultados no histórico."
            />
            <div className="mt-4 grid grid-cols-2 gap-4">
              {["Compiladores", "Engenharia de Software", "Ética em Computação"].map((d) => (
                <FormField key={d} label={`Comprovante — ${d}`} required full>
                  <Input type="file" className="h-10" />
                </FormField>
              ))}
            </div>
            <ValidationCallout className="mt-3" tone="info">
              Apenas disciplinas com comprovante e plano autorizado serão registradas no histórico.
            </ValidationCallout>
            <div className="mt-4 flex justify-end gap-2">
              <Button variant="outline" onClick={() => setView("overview")}>
                Cancelar
              </Button>
              <Button onClick={handleSendComprovantes}>Enviar comprovantes</Button>
            </div>
          </div>
        </div>
      )}
...
function Wizard({
  ...
  const handleFinalSubmit = () => {
    toast.success(
      "Candidatura enviada com sucesso! Protocolo: MOB-" + Math.floor(Math.random() * 10000),
    );
    onStep(3);
  };
      ...
      {step === 1 && (
        <div className="rounded-xl border bg-card p-6 shadow-card">
          <SectionTitle
            ...
            subtitle="Liste as disciplinas a cursar e as equivalências pretendidas."
          />
          <FormField className="mt-3" label="Disciplinas externas + equivalências" full required>
            <Textarea
              rows={6}
              placeholder="Compiladores → AED401&#10;Engenharia de Software → ES401"
            />
          </FormField>
          <ValidationCallout className="mt-3" tone="info">
            Carga horária externa deve ser ≥ à da disciplina local equivalente.
          </ValidationCallout>
          <div className="mt-4 flex justify-end gap-2">
      ...
      {step === 3 && (
        <div className="space-y-4">
          <SuccessBanner
            ...
            description="A coordenação analisará o plano em até 15 dias úteis."
          />
...
type PedidoMob = {
  ...
  periodo: string;
  disciplinas: number;
  status: "Em análise" | "Autorizada" | "Indeferida" | "Em curso";
};
...
const pedidosIniciais: PedidoMob[] = [
  ...
  },
  {
    id: "MOB-2025-019",
    aluno: "Júlia Rocha",
    instituicao: "Politecnico di Milano",
    pais: "Itália",
    periodo: "2025.2 → 2026.1",
    disciplinas: 5,
    status: "Autorizada",
  ...
  return (
    <div className="space-y-5">
      <StatsRow
        stats={[
          {
            label: "Pedidos aguardando análise",
            value: pedidos.filter((p) => p.status === "Em análise").length,
            tone: "warning",
          },
          {
            label: "Autorizadas no semestre",
            value: pedidos.filter((p) => p.status === "Autorizada").length,
            tone: "success",
          },
          {
            label: "Em curso",
            value: pedidos.filter((p) => p.status === "Em curso").length,
            tone: "info",
          },
      ...
      <ValidationCallout tone="info">
        Equivalências devem respeitar carga horária ≥ à local e estar previstas na matriz vigente.
      </ValidationCallout>
      <DataTable
        columns={[
          ...
          { key: "disciplinas", header: "Disc.", align: "right" },
          {
            ...
            render: (r) => (
              <StatusBadge
                tone={
                  r.status === "Autorizada" || r.status === "Em curso"
                    ? "success"
                    : r.status === "Indeferida"
                      ? "danger"
                      : "warning"
            ...
            render: (r) =>
              r.status === "Em análise" ? (
                <div className="flex justify-end gap-1.5">
                  ...
                  <RowActionButton tone="danger" onClick={() => updateStatus(r.id, "Indeferida")}>
                    Indeferir
                  </RowActionButton>
                  <RowActionButton onClick={() => updateStatus(r.id, "Autorizada")}>
                    Autorizar
                  </RowActionButton>
                </div>
              ) : (
      ...
      <DialogContent className="max-w-2xl">
        {pedido && (
          <>
            <DialogHeader>
              <DialogTitle>
                {pedido.id} — {pedido.aluno}
              </DialogTitle>
              <DialogDescription>
                {pedido.instituicao} ({pedido.pais}) · {pedido.periodo}
              </DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-2 gap-3 text-[13px]">
              <div>
                ...
                <StatusBadge
                  tone={
                    pedido.status === "Autorizada" || pedido.status === "Em curso"
                      ? "success"
                      : pedido.status === "Indeferida"
                        ? "danger"
                        : "warning"
                  }
                >
              ...
              <div>
                <span className="text-muted-foreground">Disciplinas no plano: </span>
                <strong>{pedido.disciplinas}</strong>
              </div>
            </div>
            <SectionTitle title="Plano de estudos" />
            <DataTable
              columns={[
                { key: "ext", header: "Externa" },
                { key: "ch", header: "CH ext.", align: "right" },
                { key: "equiv", header: "Equivalência" },
                { key: "chL", header: "CH local", align: "right" },
                {
                  key: "status",
                  header: "Status",
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() => {
                  toast.success(`Mensagem enviada a ${pedido.aluno}.`);
                  onClose();
                }}
              >
                Contatar estudante
