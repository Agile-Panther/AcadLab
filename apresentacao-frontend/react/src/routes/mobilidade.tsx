...
type PedidoMob = {
  ...
  periodo: string; disciplinas: number;
...
function Page() {
  ...
  const cancelarMob = useMutation({
    mutationFn: () =>
      api.mobilidade.solicitarCancelamento(mob?.id ?? 1, {
        justificativa: "Cancelamento solicitado pelo estudante.",
        hoje: HOJE,
      }),
    onSuccess: () => {
      if (mob) setMob({ ...mob, status: "Cancelada" });
      toast.info("Mobilidade cancelada com sucesso.");
    },
    onError: () => toast.error("Erro ao cancelar mobilidade."),
  });
    ...
    <AppShell title="Mobilidade Acadêmica" subtitle={subtitle}>
            ...
            <DataTable className="mt-3"
              columns={[
                ...
                { key: "status", header: "Status", render: (r) => (
                  <StatusBadge tone={r.status === "Aprovada" ? "success" : "warning"}>{r.status}</StatusBadge>
                )},
              ]}
              rows={[
          ...
          <div className="flex flex-wrap gap-2">
            {mob.status === "Autorizada" && (
              <>
                ...
                <Button
                  variant="destructive"
                  onClick={() => cancelarMob.mutate()}
                  disabled={cancelarMob.isPending}
                >
                  {cancelarMob.isPending ? "Cancelando…" : "Cancelar mobilidade"}
                </Button>
              </>
            )}
            {mob.status === "Em curso" && (
      ...
      {view === "wizard" && (
        <Wizard
          ...
          onDone={() => {
            setView("overview");
            setMob({ id: 1, instituicao: "Nova", pais: "—", inicio: "—", fim: "—", status: "Em análise" });
          }}
        />
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border bg-card p-6 shadow-card">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <Button variant="outline" onClick={() => setView("overview")}>Cancelar</Button>
...
function Wizard({ step, onStep, onDone }: {
  step: 0 | 1 | 2 | 3;
      ...
      {step === 0 && (
        <div className="rounded-xl border bg-card p-6 shadow-card">
          ...
          <div className="mt-4 flex justify-end">
            <Button disabled={!instituicao} onClick={() => onStep(1)}>Avançar</Button>
          </div>
        </div>
      )}
      ...
      {step === 1 && (
        <div className="rounded-xl border bg-card p-6 shadow-card">
          ...
          <FormField className="mt-3" label="Disciplinas externas + equivalências" full required>
            <Textarea rows={6} placeholder={"Compiladores → AED401\nEngenharia de Software → ES401"} />
          </FormField>
          <ValidationCallout className="mt-3" tone="info">
...
function CoordView() {
  ...
  return (
    <div className="space-y-5">
      <StatsRow stats={[
        { label: "Aguardando análise", value: pedidos.filter((p) => p.status === "Em análise").length, tone: "warning" },
      ...
      <DataTable
        columns={[
          ...
          { key: "status", header: "Status", render: (r) => (
            <StatusBadge tone={r.status === "Autorizada" || r.status === "Em curso" ? "success" : r.status === "Indeferida" ? "danger" : "warning"}>
              {r.status}
            </StatusBadge>
          )},
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3 text-[13px]">
              <div>
                ...
                <StatusBadge tone={pedido.status === "Autorizada" || pedido.status === "Em curso" ? "success" : pedido.status === "Indeferida" ? "danger" : "warning"}>
                  {pedido.status}
                </StatusBadge>
              </div>
            ...
            <DataTable
              columns={[
                ...
                { key: "chL", header: "CH local", align: "right" },
                { key: "status", header: "Status", render: (r) => (
                  <StatusBadge tone={r.status === "Aprovada" ? "success" : r.status === "Em análise" ? "warning" : "danger"}>
                    {r.status}
                  </StatusBadge>
                )},
              ]}
            ...
            <DialogFooter>
              ...
              <Button onClick={() => { toast.success(`Mensagem enviada a ${pedido.aluno}.`); onClose(); }}>
                Contatar estudante
              </Button>
            </DialogFooter>
          </>
