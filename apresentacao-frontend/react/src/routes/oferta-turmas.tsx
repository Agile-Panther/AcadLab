...
function Page() {
  const { active: perfil } = useProfileSwitcher([
    { value: "coordenacao", label: "Coordenação Acadêmica", description: "Planeja e abre turmas" },
    {
      ...
      description: "Aloca salas e publica oferta",
    },
  ]);
        ...
        <div className="space-y-5">
          ...
          {isSec ? (
            ...
            <ActionBar
              ...
              primaryLabel="Nova turma"
              onPrimary={() => setWizard(0)}
            />
          )}
          <DataTable
            columns={[
              ...
              },
              {
                ...
                render: (r) =>
                  isSec ? (
                    <RowActionButton
                      onClick={() => {
                        toast.success(`Sala ${r.sala} confirmada para ${r.codigo}.`);
                      }}
                    >
                      <MapPin className="mr-1 h-3 w-3 inline" /> Alocar sala
                    </RowActionButton>
                  ) : (
      ...
      {tab === "publicar" && isSec && (
        <div className="space-y-4">
          <div className="rounded-xl border bg-card p-6 shadow-card">
            <SectionTitle
              title="Publicação da oferta 2025.2"
              subtitle="Após publicada, as turmas ficam visíveis para os estudantes na matrícula."
            />
            ...
            <ValidationCallout className="mt-4" tone="info">
              Publicar gera notificação a todos os estudantes elegíveis e abre a janela de
              matrícula.
            </ValidationCallout>
            <div className="mt-4 flex justify-end">
              <Button
                ...
                onClick={() => {
                  setPublicada(true);
                  toast.success("Oferta 2025.2 publicada com sucesso.");
                }}
              >
                <Send className="mr-2 h-4 w-4" />{" "}
                {publicada ? "Oferta publicada" : "Publicar oferta"}
              </Button>
            </div>
          </div>
        </div>
        ...
        <SheetContent className="w-[480px] sm:max-w-md">
          {selected && (
            <>
              <SheetHeader>
                <SheetTitle>
                  {selected.codigo} — {selected.disciplina}
                </SheetTitle>
              </SheetHeader>
              ...
              <ValidationCallout className="mt-4" tone="info">
                Cancelar gera notificação aos {parseInt(selected.vagas)} estudantes matriculados.
              </ValidationCallout>
            </>
          )}
        </SheetContent>
      ...
      <DialogContent>
        {turma && (
          <>
            <DialogHeader>
              <DialogTitle>
                Editar turma {turma.codigo} — {turma.turma}
              </DialogTitle>
              <DialogDescription>{turma.disciplina}</DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Professor" full>
                <Input className="h-10" value={prof} onChange={(e) => setProf(e.target.value)} />
              </FormField>
              <FormField label="Sala">
                <Input className="h-10" value={sala} onChange={(e) => setSala(e.target.value)} />
              </FormField>
              ...
              <FormField label="Vagas (ocupadas/total)" full>
                <Input className="h-10" value={vagas} onChange={(e) => setVagas(e.target.value)} />
              </FormField>
            </div>
            <ValidationCallout tone="info">
              Alterações geram notificação aos estudantes matriculados.
            </ValidationCallout>
            <DialogFooter>
      ...
      <DataTable
        columns={[
          ...
          { key: "tipo", header: "Tipo" },
          {
            ...
            header: "Status",
            render: (r) => (
              <StatusBadge tone={r.status === "Ativa" ? "success" : "neutral"}>
                {r.status}
              </StatusBadge>
            ),
          },
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <FormField label="Tipo">
                  <Input className="h-10" value={tipo} onChange={(e) => setTipo(e.target.value)} />
                </FormField>
                <FormField label="Status" full>
                  <div className="flex gap-2">
                    ...
                    <Button
                      type="button"
                      size="sm"
                      variant={status === "Inativa" ? "default" : "outline"}
    ...
    <>
      <DataTable
        columns={[
          ...
          { key: "depto", header: "Departamento" },
          { key: "turmas", header: "Turmas vinculadas", align: "right" },
          {
            key: "status",
            header: "Status",
            render: (r) => (
              <StatusBadge tone={r.status === "Ativo" ? "success" : "neutral"}>
                {r.status}
              </StatusBadge>
            ),
          },
          ...
          {edit && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {edit.nome} · {edit.turmas} turmas vinculadas
                </DialogDescription>
              </DialogHeader>
              <div className="grid grid-cols-2 gap-3">
                <FormField label="Departamento" full>
                  <Input
                    className="h-10"
                    value={depto}
                ...
                <FormField label="E-mail institucional" full>
                  <Input
                    className="h-10"
                    type="email"
                    value={email}
        ...
        {step === 0 && (
          <>
            <SectionTitle
              ...
              subtitle="Apenas disciplinas da matriz ativa."
            />
            <div className="mt-4 grid grid-cols-2 gap-4">
              <FormField label="Disciplina" required>
                <Input className="h-10" placeholder="ES401 — Arquitetura de Software" />
              </FormField>
              <FormField label="Turma">
                <Input className="h-10" defaultValue="T01" />
              </FormField>
            </div>
          </>
        )}
          ...
          <>
            ...
            <div className="mt-4 grid grid-cols-2 gap-4">
              <FormField label="Professor" required>
                <Input className="h-10" placeholder="Carlos Lima" />
              </FormField>
              <FormField label="Sala" required>
                <Input className="h-10" placeholder="B-203" />
              </FormField>
              <FormField label="Horário" required full>
                <Input className="h-10" placeholder="Seg/Qua 08:00-10:00" />
              </FormField>
              <FormField label="Capacidade" required>
                <Input type="number" className="h-10" defaultValue={30} />
              </FormField>
            </div>
            <ValidationCallout className="mt-4" tone="info">
              Sem conflito de horário detectado para professor e sala.
            </ValidationCallout>
          </>
        )}
        {step === 2 && (
          <>
            ...
            <p className="mt-3 text-[13px] text-muted-foreground">
              Confirme a oferta da turma. Após confirmar, ela ficará visível na matrícula.
            </p>
          </>
        )}
        <div className="mt-4 flex justify-end gap-2">
          {step > 0 && (
            <Button variant="outline" onClick={() => onStep((step - 1) as 0 | 1)}>
              Voltar
            </Button>
          )}
