...
import {
  useCasosEstudante, useCasosResponsavel, useCasosAbertos,
  useSolicitarApoio, useRealizarTriagem, useRegistrarAtendimento, useEncerrarCaso,
  useReabrirCaso, useAgendar, useContestarAgendamento,
  type CasoResumo, type StatusCaso, type PrioridadeTriagem, type StatusAgendamento,
} from "@/lib/apoio";
...
function statusTone(s: StatusCaso) {
  return s === "EM_ATENDIMENTO" ? "success" as const : s === "ENCERRADO" ? "neutral" as const : "warning" as const;
}
...
const responsavelDe = (c: CasoResumo) => (c.responsavelId ? `Psicopedagogo #${c.responsavelId}` : "A definir");
...
function EstudanteView() {
  ...
  const handleContestar = () => {
    if (!caso) return;
    if (!justificativa.trim()) { toast.error("Informe o motivo da troca."); return; }
    contestar.mutate(
      { casoId: caso.id, justificativa, horarioSugerido: horarioSugerido || null },
      {
        onSuccess: () => {
          toast.success("Pedido de troca de horário enviado.");
          setContestando(false); setJustificativa(""); setHorarioSugerido("");
        },
        onError: notifyError,
      },
    );
  ...
  const handleEnviar = () => {
    ...
    solicitar.mutate(texto, {
      onSuccess: () => { toast.success("Solicitação enviada para triagem!"); resetForm(); setView("enviado"); },
      onError: notifyError,
    });
  };
  ...
  const handleReabrir = () => {
    ...
    reabrir.mutate(caso.id, {
      onSuccess: () => { toast.info("Caso reaberto para acompanhamento."); setView("list"); setSelectedId(null); },
      onError: notifyError,
    });
  };
    ...
    return (
      <div className="space-y-5">
        ...
        <StatsRow stats={[
          ...
          { label: "Em triagem", value: casos.filter((c) => c.status === "ABERTO").length, tone: "warning" },
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-0.5 text-sm font-semibold text-foreground">{motivoDe(c)}</p>
                        </div>
                      ...
                      <div className="mt-3 space-y-1 text-[12px] text-muted-foreground">
                        <p>Aberto em {formatData(c.abertura)} · {responsavelDe(c)}</p>
                        {c.agendamento && (
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  {encerrados.map((c) => (
                    ...
                    >
                      ...
                      <p className="mt-3 text-[12px] text-muted-foreground">Aberto em {formatData(c.abertura)} · {responsavelDe(c)}</p>
                    </button>
                  ))}
                </div>
              </div>
  ...
  if (view === "detail" && caso) {
    return (
      <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <SectionTitle title={motivoDe(caso)} subtitle={`PSI-${caso.id} · Aberto em ${formatData(caso.abertura)} · Responsável: ${responsavelDe(caso)}`} />
              ...
              <ol className="mt-4 space-y-3 border-l border-border pl-5">
                {caso.atendimentos.map((t, i) => (
                  <li key={i} className="relative">
                    ...
                    <p className="text-[13px] font-medium text-foreground">{t.conclusaoFinal ? "Atendimento final" : "Atendimento realizado"}</p>
                    {t.observacoes && <p className="text-[12px] text-muted-foreground">{t.observacoes}</p>}
                    {t.encaminhamento && <p className="text-[12px] text-muted-foreground">Encaminhamento: {t.encaminhamento}</p>}
                  </li>
                ))}
              </ol>
            )}
          ...
          <div className="space-y-3">
            {caso.agendamento && (
              <div className="rounded-xl border bg-card p-5 shadow-card space-y-2">
                <h3 className="font-semibold flex items-center gap-2"><CalendarClock className="h-4 w-4" /> Próximo atendimento</h3>
                <p className="text-[13px] text-foreground">{formatDataHora(caso.agendamento.dataHora)}</p>
                <StatusBadge tone={caso.agendamento.status === "AGENDADO" ? "success" : "warning"}>{agendamentoLabel(caso.agendamento.status)}</StatusBadge>
                {caso.agendamento.status === "CONTESTADO" ? (
                  <p className="text-[12px] text-muted-foreground">
                    Troca solicitada{caso.agendamento.justificativaContestacao ? `: ${caso.agendamento.justificativaContestacao}` : "."} Aguarde o reagendamento.
                  </p>
                ) : (
                  <Button variant="outline" className="w-full" onClick={() => setContestando(true)}>Solicitar troca de horário</Button>
                )}
              </div>
            )}
            <ValidationCallout tone="info">Os registros são sigilosos. Apenas você e a profissional responsável têm acesso.</ValidationCallout>
            {caso.status === "ENCERRADO" && (
              <Button className="w-full" disabled={reabrir.isPending} onClick={handleReabrir}>Reabrir caso</Button>
            )}
          </div>
        </div>
        ...
        <Dialog open={contestando} onOpenChange={(o) => !o && setContestando(false)}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Explique o motivo e, se quiser, sugira um horário melhor para você.</DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-2">
              <FormField label="Motivo" required full><Textarea rows={3} value={justificativa} onChange={(e) => setJustificativa(e.target.value)} placeholder="Ex.: tenho aula nesse horário" /></FormField>
              <FormField label="Horário sugerido (opcional)" full><Input type="datetime-local" className="h-10" min={agoraParaInput()} value={horarioSugerido} onChange={(e) => setHorarioSugerido(e.target.value)} /></FormField>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setContestando(false)}>Cancelar</Button>
              <Button disabled={contestar.isPending} onClick={handleContestar}>Enviar pedido</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border bg-card p-6 shadow-card">
          ...
          <div className="mt-4 flex justify-end gap-2">
            ...
            <Button disabled={solicitar.isPending} onClick={handleEnviar}>Enviar solicitação</Button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      <SuccessBanner title="Solicitação registrada!" description="Sua demanda foi encaminhada para triagem. Você receberá retorno em breve." />
...
function PsicoView() {
  ...
  const confirmAgendar = () => {
    ...
    if (!dataHoraAgendar) { toast.error("Escolha a data e hora do atendimento."); return; }
    agendar.mutate({ casoId: agendarCaso.id, dataHora: dataHoraAgendar }, {
      onSuccess: () => { toast.success("Horário agendado."); setAgendarCaso(null); setDataHoraAgendar(""); },
      onError: notifyError,
    });
  };
  ...
  const confirmTriagem = () => {
    ...
    triar.mutate({ casoId: triagemCaso.id, prioridade, observacoes: obsTriagem }, {
      onSuccess: () => { toast.success("Triagem registrada."); setTriagemCaso(null); setObsTriagem(""); setPrioridade("MEDIA"); },
      onError: notifyError,
    });
  };
  ...
  const confirmAtendimento = () => {
    ...
    if (!obsAtend.trim()) { toast.error("Descreva as observações do atendimento."); return; }
    atender.mutate({
      ...
      onSuccess: () => { toast.success("Atendimento registrado."); setAtendimentoCaso(null); setObsAtend(""); setEncaminhamento(""); setConclusaoFinal(false); },
      onError: notifyError,
    });
  };
  ...
  return (
    <div className="space-y-5">
      <StatsRow stats={[
        { label: "Casos em triagem", value: fila.filter((c) => c.status === "ABERTO").length, tone: "warning" },
        { label: "Em acompanhamento", value: fila.filter((c) => c.status === "EM_ATENDIMENTO").length, tone: "info" },
        { label: "Alta prioridade", value: fila.filter((c) => c.prioridadeTriagem === "ALTA" || c.prioridadeTriagem === "URGENTE").length, tone: "danger" },
        { label: "Encerrados", value: fila.filter((c) => c.status === "ENCERRADO").length, tone: "success" },
      ]} />
      ...
      {contestacoes.length > 0 && (
        <div className="rounded-xl border bg-card p-5 shadow-card space-y-3">
          <div className="flex items-center gap-2">
            <SectionTitle title="Solicitações de troca de horário" subtitle="Alunos pediram remarcação do atendimento" />
            <StatusBadge tone="warning">{contestacoes.length}</StatusBadge>
          </div>
          <div className="space-y-2">
            {contestacoes.map((c) => (
              <div key={c.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-subtle p-3">
                <div className="text-[13px]">
                  <p className="font-medium text-foreground">PSI-{c.id} · Estudante #{c.estudanteId} · {motivoDe(c)}</p>
                  <p className="text-[12px] text-muted-foreground">
                    Horário atual: {formatDataHora(c.agendamento!.dataHora)}
                    {c.agendamento!.horarioSugerido ? ` · sugerido pelo aluno: ${formatDataHora(c.agendamento!.horarioSugerido)}` : ""}
                  </p>
                  {c.agendamento!.justificativaContestacao && (
                    <p className="text-[12px] text-muted-foreground">Motivo: {c.agendamento!.justificativaContestacao}</p>
                  )}
                </div>
                <Button
                  size="sm"
                  onClick={() => { setAgendarCaso(c); setDataHoraAgendar(c.agendamento!.horarioSugerido?.slice(0, 16) ?? ""); }}
                >
                  Reagendar
                </Button>
              </div>
      ...
      {loading ? (
        ...
        <ValidationCallout tone="info">Nenhum caso sob sua responsabilidade no momento.</ValidationCallout>
      ) : (
        <DataTable
          columns={[
            ...
            { key: "prioridade", header: "Prioridade", render: (r) => <StatusBadge tone={prioridadeTone(r.prioridadeTriagem)}>{prioridadeLabel(r.prioridadeTriagem)}</StatusBadge> },
            ...
            { key: "acoes", header: "", align: "right", render: (r) => (
              <div className="flex justify-end gap-1.5">
                {r.status === "ABERTO" && !r.prioridadeTriagem && <RowActionButton onClick={() => setTriagemCaso(r)}>Triar</RowActionButton>}
                {r.status !== "ENCERRADO" && (!!r.prioridadeTriagem || r.status === "EM_ATENDIMENTO") && (
                  <RowActionButton tone="info" onClick={() => setAtendimentoCaso(r)}>Atender</RowActionButton>
                )}
                {r.status !== "ENCERRADO" && (
                  <RowActionButton tone="neutral" onClick={() => { setAgendarCaso(r); setDataHoraAgendar(""); }}>
                    {r.agendamento ? "Reagendar" : "Agendar"}
                  </RowActionButton>
                )}
                {r.status === "EM_ATENDIMENTO" && <RowActionButton tone="danger" onClick={() => handleEncerrar(r)}>Encerrar</RowActionButton>}
                <RowActionButton tone="neutral" onClick={() => setProntuario(r)}>Prontuário</RowActionButton>
              </div>
            )},
          ]}
          rows={fila}
      ...
      <Dialog open={!!triagemCaso} onOpenChange={(o) => !o && setTriagemCaso(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{triagemCaso && `PSI-${triagemCaso.id} · ${motivoDe(triagemCaso)}`}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <FormField label="Observações" full><Textarea rows={3} value={obsTriagem} onChange={(e) => setObsTriagem(e.target.value)} placeholder="Anotações da triagem..." /></FormField>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setTriagemCaso(null)}>Cancelar</Button>
            <Button disabled={triar.isPending} onClick={confirmTriagem}>Confirmar triagem</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!atendimentoCaso} onOpenChange={(o) => !o && setAtendimentoCaso(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{atendimentoCaso && `PSI-${atendimentoCaso.id} · ${motivoDe(atendimentoCaso)}`}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <FormField label="Observações" required full><Textarea rows={3} value={obsAtend} onChange={(e) => setObsAtend(e.target.value)} placeholder="Registro do atendimento..." /></FormField>
            <FormField label="Encaminhamento (opcional)" full><Input className="h-10" value={encaminhamento} onChange={(e) => setEncaminhamento(e.target.value)} placeholder="Ex.: Psicólogo, Pedagogo, Externo" /></FormField>
            <label className="flex items-center gap-2 text-[13px] text-foreground">
              <input type="checkbox" checked={conclusaoFinal} onChange={(e) => setConclusaoFinal(e.target.checked)} />
              Registrar como conclusão final do caso
            </label>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAtendimentoCaso(null)}>Cancelar</Button>
            <Button disabled={atender.isPending} onClick={confirmAtendimento}>Salvar atendimento</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!agendarCaso} onOpenChange={(o) => !o && setAgendarCaso(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{agendarCaso?.agendamento ? "Reagendar horário" : "Agendar horário"}</DialogTitle>
            <DialogDescription>{agendarCaso && `PSI-${agendarCaso.id} · Estudante #${agendarCaso.estudanteId}`}</DialogDescription>
          </DialogHeader>
          {agendarCaso?.agendamento?.status === "CONTESTADO" && (
            <ValidationCallout tone="info">
              ...
              {agendarCaso.agendamento.horarioSugerido ? ` · sugeriu ${formatDataHora(agendarCaso.agendamento.horarioSugerido)}` : ""}
            </ValidationCallout>
          )}
          <div className="space-y-4 py-2">
            <FormField label="Data e hora" required full>
              <Input type="datetime-local" className="h-10" min={agoraParaInput()} value={dataHoraAgendar} onChange={(e) => setDataHoraAgendar(e.target.value)} />
            </FormField>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAgendarCaso(null)}>Cancelar</Button>
            <Button disabled={agendar.isPending} onClick={confirmAgendar}>Confirmar horário</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {prontuario && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>Estudante #{prontuario.estudanteId} · Aberto em {formatData(prontuario.abertura)}</DialogDescription>
              </DialogHeader>
              <ValidationCallout tone="info">Documento sigiloso. Acesso registrado em log de auditoria do NAP.</ValidationCallout>
              <div className="space-y-2 text-[13px]">
                <div className="flex justify-between border-b py-2"><span className="text-muted-foreground">Tema</span><span className="font-medium">{motivoDe(prontuario)}</span></div>
                <div className="flex justify-between border-b py-2"><span className="text-muted-foreground">Prioridade</span><StatusBadge tone={prioridadeTone(prontuario.prioridadeTriagem)}>{prioridadeLabel(prontuario.prioridadeTriagem)}</StatusBadge></div>
                <div className="flex justify-between border-b py-2"><span className="text-muted-foreground">Status</span><StatusBadge tone={statusTone(prontuario.status)}>{statusLabel(prontuario.status)}</StatusBadge></div>
              </div>

              {prontuario.triagemObservacoes && (
                <div className="rounded-lg border bg-subtle p-3">
                  <p className="text-[12px] font-semibold text-muted-foreground">Observações da triagem</p>
                  <p className="mt-1 text-[13px] text-foreground">{prontuario.triagemObservacoes}</p>
                </div>
              )}
              ...
              {prontuario.agendamento && (
                <div className="rounded-lg border p-3 flex items-start justify-between gap-3">
                  <div>
                    <p className="text-[12px] font-semibold text-muted-foreground flex items-center gap-1.5"><CalendarClock className="h-3.5 w-3.5" /> Próximo atendimento</p>
                    <p className="mt-1 text-[13px] text-foreground">{formatDataHora(prontuario.agendamento.dataHora)}</p>
                    {prontuario.agendamento.status === "CONTESTADO" && (
                      <p className="mt-1 text-[12px] text-muted-foreground">
                        Troca pedida: {prontuario.agendamento.justificativaContestacao}
                        {prontuario.agendamento.horarioSugerido ? ` · sugeriu ${formatDataHora(prontuario.agendamento.horarioSugerido)}` : ""}
                      </p>
                    )}
                  </div>
                  <StatusBadge tone={prontuario.agendamento.status === "AGENDADO" ? "success" : "warning"}>{agendamentoLabel(prontuario.agendamento.status)}</StatusBadge>
                </div>
              )}

              {prontuario.status !== "ENCERRADO" && (
                <Button variant="outline" className="w-full" onClick={() => { const c = prontuario; setProntuario(null); setAgendarCaso(c); setDataHoraAgendar(""); }}>
              ...
              {prontuario.atendimentos.length === 0 ? (
                ...
                <ul className="space-y-2 text-[13px] text-muted-foreground">
                  {prontuario.atendimentos.map((a, i) => (
                    <li key={i}>• {formatData(a.data)} — {a.observacoes ?? "Atendimento"}{a.encaminhamento ? ` (encaminhado: ${a.encaminhamento})` : ""}{a.conclusaoFinal ? " · conclusão final" : ""}</li>
                  ))}
                </ul>
              )}
              <DialogFooter>
