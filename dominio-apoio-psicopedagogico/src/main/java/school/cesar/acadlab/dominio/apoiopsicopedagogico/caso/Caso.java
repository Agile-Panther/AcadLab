...
public class Caso {
    ...
    public Caso(CasoId id, EstudanteId estudanteId, PsicopedagogoId responsavelId,
                StatusCaso status, Triagem triagem, Agendamento agendamento,
    ...
    // RN2: triagem registrada; define responsável e inicia o acompanhamento do caso
    ...
    // Psicopedagogo marca/reagenda um horário de atendimento com o aluno
    public AgendamentoMarcadoEvento agendar(LocalDateTime dataHora, LocalDateTime agora) {
        ...
        if (status == StatusCaso.ENCERRADO) {
            throw new IllegalStateException("não é possível agendar horário em um caso encerrado");
        }
        if (!dataHora.isAfter(agora)) {
            throw new IllegalStateException("o horário do atendimento deve ser no futuro");
        }
        this.agendamento = new Agendamento(dataHora);
        return new AgendamentoMarcadoEvento(this);
    }

    // Aluno contesta o horário e solicita troca (justificativa obrigatória; horário sugerido opcional)
    public AgendamentoContestadoEvento contestarAgendamento(String justificativa, LocalDateTime horarioSugerido, LocalDateTime agora) {
        notNull(agora, "O instante de referência não pode ser nulo");
        if (agendamento == null) {
            throw new IllegalStateException("não há horário agendado para contestar");
        }
        if (agendamento.estaContestado()) {
            throw new IllegalStateException("o horário já foi contestado; aguarde o reagendamento");
        }
        if (horarioSugerido != null && !horarioSugerido.isAfter(agora)) {
            throw new IllegalStateException("o horário sugerido deve ser no futuro");
        }
        this.agendamento = agendamento.contestar(justificativa, horarioSugerido);
        return new AgendamentoContestadoEvento(this);
    }
    ...
    public static class AgendamentoMarcadoEvento extends CasoEvento {
        private AgendamentoMarcadoEvento(Caso caso) { super(caso); }
    }

    public static class AgendamentoContestadoEvento extends CasoEvento {
        private AgendamentoContestadoEvento(Caso caso) { super(caso); }
    }
}
